...
request.onload = function () {
  ...
  function displayPage(queryset, page, rows) {
    let Start = (page - 1) * rows;
    let End = Start + rows;
    let getData = queryset.slice(Start, End)
    let pages = Math.ceil(pagedata.length / rows);
    return {
      "queryset": getData,
      "pages": pages
    }
  ...
  function buildTable() {
    let data = displayPage(state.queryset, state.page, state.rows)
